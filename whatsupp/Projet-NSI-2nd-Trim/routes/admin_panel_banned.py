...
def user_banned():
    ...
    with Session(engine) as session:
        ...
        if not session_cookie or not user or not user.admin:
            return redirect(url_for('login_get'))

    return render_template('high_admin_panel/bannissements.html')

def user_banned_post(userId):
    session_cookie = request.cookies.get('session_cookie')
    ...
    with Session(engine) as session:
        ...
        user = session.exec(statement).one_or_none()
        if not user:
            return redirect(url_for('user_banned'))
