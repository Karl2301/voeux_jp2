...
def delete_message():
    ...
    statement = select(Conversation).where(Conversation.id == message_id)
    ...
    # Emission de l'événement WebSocket pour informer les utilisateurs
    socketio.emit('message_deleted', {
        'message_id': message_id,
        'user_id': str(user.id),
        'contact_id': str(message.contact_id)
    }, room=[str(user.id), str(message.contact_id)])
